...
import { DOMAIN_COLORS, DOMAIN_METRIC_LABELS, DOMAIN_KEY_METRICS } from '../lib/colors'
...
function buildChartData(
  ...
  const len = Math.min(sleepRows.length, activityRows.length, nutritionRows.length, stressRows.length, 14)
...
const LINES = [
  ...
  { key: 'nutrition', color: DOMAIN_COLORS.nutrition, label: 'Cal Deficit /10' },
...
function CustomTooltip({ active, payload, label }: any) {
  ...
  return (
    <div style={{
      background: '#1a1d2b', border: '0.5px solid #2a2d3a',
...
export default function Timeline() {
  ...
  return (
    <div style={{
      ...
      borderRadius: 14, padding: '16px', height: '100%',
    }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
        <span style={{ fontSize: 13, fontWeight: 500, color: '#e8eaf0' }}>Health Timeline</span>
      ...
      {loading ? (
        ...
        <ResponsiveContainer width="100%" height={220}>
          <LineChart data={view === 'timeline' ? chartData : weeklyData} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
      ...
      <div style={{ marginTop: 12, padding: '8px 12px', background: '#BA751710', borderRadius: 8, border: '0.5px solid #BA751730' }}>
        <span style={{ fontSize: 11, color: '#BA7517' }}>
          Tip: Switch to <strong>weekly</strong> view to see the Tuesday pattern — deep sleep drops to ~12% vs 22% every other day.
        </span>
      </div>
    </div>
  )
